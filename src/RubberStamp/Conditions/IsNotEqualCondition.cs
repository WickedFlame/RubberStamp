...
{
    public class IsNotEqualCondition<T, TProp> : PropertyValidationCondition<T, TProp> 
    {
        private readonly TProp _value;

        public IsNotEqualCondition(TProp value)
        {
            _value = value;
        }

        public IsNotEqualCondition(Expression<Func<T, TProp>> property, TProp value)
            : this(value)
        {
            Property = property;
        }
        ...
        {
            return $"The Property {Property.TryExtractPropertyName()} has to be unequal to {_value}";
        }
    }
}
