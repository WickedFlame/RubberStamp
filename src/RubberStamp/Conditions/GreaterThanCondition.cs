...
{
    public class GreaterThanCondition<T, TProp> : PropertyValidationCondition<T, TProp>
    {
        private readonly TProp _value;

        public GreaterThanCondition(TProp value)
        {
            _value = value;
        }

        public GreaterThanCondition(Expression<Func<T, TProp>> property, TProp value)
            : this(value)
        {
            Property = property;
        }
        ...
        {
            return $"The Property {Property.TryExtractPropertyName()} has to be greater than {_value}";
        }
    }
}
