...
{
    public class LessOrEqualThanCondition<T, TProp> : PropertyValidationCondition<T, TProp> 
    {
        private readonly TProp _value;

        public LessOrEqualThanCondition(TProp value)
        {
            _value = value;
        }

        public LessOrEqualThanCondition(Expression<Func<T, TProp>> property, TProp value)
            : this(value)
        {
            Property = property;
        }
        ...
        {
            return $"The Property {Property.TryExtractPropertyName()} has to be less than or equal to { _value}";
        }
    }
}
